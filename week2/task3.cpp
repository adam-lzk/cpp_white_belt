// Вывод строк-палиндромов длиной не менее minLength
...
vector<string> PalindromFilter (vector<string> words, int minLength)
{
    ...
    for (string element : words)  // (int i = 0; i < words.size(); i++)
    {
        ...
        if (element.size() >= minLength)
        {
            ...
            if (check == 0)  // заполнение вектора filtredWords строками-палиндромами длиной не менее minLength
...
int main()
{
    ...
    for (int i = 0; i < 5; i++)  // заполнение вектора words 5-ю словами
    {
        cout << "words " << i << " = ";
        cin >> str;
        words.push_back(str);
